...
SELECT DISTINCT urun_adi FROM market;
...
SELECT urun_adi , SUM(miktar_kg) FROM market GROUP BY urun_adi;
...
SELECT urun_adi, SUM(miktar_kg) as satilma_miktari FROM market GROUP BY urun_adi ORDER BY satilma_miktari DESC LIMIT 1;
...
SELECT satis_tarihi, AVG(satis_fiyati) as ortalama_satis_fiyati FROM market GROUP BY satis_tarihi;
...
SELECT * FROM market ORDER BY satis_fiyati DESC LIMIT 1;
...
SELECT SUM(miktar_kg) as 'Meyve kategorisinin Toplam satis miktari' FROM market WHERE kategori = 'meyve';
...
SELECT urun_adi, AVG(satis_fiyati) FROM market GROUP BY urun_adi;
...
SELECT kategori , MAX(satis_fiyati) as 'Her bir kategorideki en yuksek fiyat' FROM market GROUP BY kategori;
